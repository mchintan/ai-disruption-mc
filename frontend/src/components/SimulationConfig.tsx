  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-50 border border-amber-200 dark:bg-amber-500/10 dark:border-amber-500/20 mb-6">
          <Settings className="w-4 h-4 text-amber-600 dark:text-amber-400" />
          <span className="text-xs font-mono font-semibold text-amber-600 dark:text-amber-400 tracking-wide">
        ...
        <div className="col-span-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div className="text-xs text-stone-500 dark:text-slate-500 font-mono">dS/S = &#956;dt + &#963;dW</div>
            ...
            >
              ...
              <div className="text-xs text-stone-500 dark:text-slate-500 font-mono">dS/S = (&#956;-&#955;k)dt + &#963;dW + JdN</div>
      ...
      {/* Portfolio Summary */}
      <div className="bg-white border border-stone-200 dark:bg-slate-800/30 dark:border-slate-700/30 rounded-xl p-4 mb-6">
        <div className="text-xs font-mono text-stone-500 dark:text-slate-500 uppercase tracking-wider mb-3">Portfolio Summary</div>
        <div className="flex flex-wrap gap-2">
          {assets.map((asset, i) => (
            <div key={i} className="bg-stone-50 border border-stone-200 dark:bg-slate-900/50 dark:border-slate-700/50 rounded-lg px-3 py-1.5">
              <span className={`text-xs font-mono font-bold ${ASSET_TEXT_COLORS[i % ASSET_TEXT_COLORS.length]}`}>{asset.ticker}</span>
              <span className="text-xs text-stone-400 dark:text-slate-500 ml-2">{asset.allocation_pct}%</span>
            </div>
          ))}
        </div>
        <div className="mt-3 text-xs text-stone-400 dark:text-slate-500 font-mono">
      ...
      <div className="flex gap-3">
        ...
        <button onClick={handleRun} disabled={isLoading} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold text-sm flex items-center justify-center gap-2 hover:from-amber-400 hover:to-orange-400 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
          {isLoading ? (<><Loader2 className="w-4 h-4 animate-spin" />Running Simulation...</>) : (<><Play className="w-4 h-4" />Run Monte Carlo<ArrowRight className="w-4 h-4" /></>)}
        </button>
      </div>
    </div>
  );
